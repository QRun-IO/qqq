...
- `InputSource inputSource` - Defaults to `QInputSource.SYSTEM` for direct Java calls. Javalin and Slack widget entry points set `QInputSource.USER`. Custom renderers should carry this source into data actions and enforce the table permissions they require; client query parameters do not set it.
