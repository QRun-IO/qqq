...
By default, `MetaDataProducerHelper` logs a warning and skips any class that fails while being
evaluated as a producer (for example, its constructor throws), and any producer whose `produce`
method throws.  The application then starts without that meta-data.
