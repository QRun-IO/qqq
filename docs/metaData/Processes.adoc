...
*** 2) by multiple calls to `.withStep(QStepMetaData)`, which adds a step to both the `stepList` and `steps` map.
** If a process also needs optional steps (for a <<_custom_process_flow>>), they should be added by a call to `.withOptionalStep(QStepMetaData)`, which only places them in the `steps` map.
...
** `STATE_MACHINE` - steps are executed as a Finite State Machine, starting with the first step in `stepList`,
...
The `supplementalMetaData` map attaches module-specific process settings. For application APIs, see {qqq-source-url}/qqq-middleware-api/src/main/java/com/kingsrook/qqq/api/model/metadata/processes/ApiProcessMetaData.java[ApiProcessMetaData].
...
Expects `downloadFileName` and either `serverFilePath` or the pair `storageTableName` / `storageReference`.
In 4.0, trusted backend code must register the generated file with `ProcessFileDownload` before returning these values; see <<ProcessDownloadGrants>>.
...
For a field configured by the server, set `isEditable = false`.
When `RunProcessInput.inputSource` is `QInputSource.USER`, caller-supplied values for those fields are rejected before saved process state is restored or merged.
Metadata defaults and previously stored backend values remain available.
** `recordListMetaData` - *RecordListMetaData object* - Identifies the table whose records a backend step needs.
If the step has no records, the framework queries that table using input record IDs or the process callback's filter.
This automatic query inherits the process input source; `USER` calls require READ permission on the table and joined tables.

The legacy Javalin process endpoints and v1 process init/step endpoints mark incoming calls as `QInputSource.USER`.
Application API process calls retain the SYSTEM default and their configured API parameter mapping and customizers; changing this provenance is deferred in https://github.com/QRun-IO/qqq/issues/451[#451].
Direct Java calls default to `QInputSource.SYSTEM`; code forwarding external input must explicitly select `USER`.
This setting governs process input validation and automatic record loading.
Queries or mutations created inside a custom backend step choose their own input source and permissions independently.
On USER process inputs, supplied values are treated as caller input when validating non-editable fields.
Application API result ownership is bound independently of process input source; restoring SYSTEM provenance does not make asynchronous results public.

Application API process custom permission checkers now run once after API input mapping, metadata defaults and `PreRunApiProcessCustomizer`, before synchronous or asynchronous process execution. This lets the built-in `ReportProcessPermissionChecker` authorize the effective `reportName`. Standard process permission checks still reject denied calls before the pre-run customizer. Customizers run before input-dependent authorization; they must authorize any sensitive side effects themselves.

==== Process state ownership

HTTP process state and asynchronous jobs belong to the originating application instance, authentication session, backend variants and process.
Status, record retrieval, resume, cancellation and process-dependent possible-value lookups require that scope and current process permission.
Clients must retain the server-issued session cookies or the same authentication reference across requests; a process or job UUID alone does not grant access.
Rejected requests are checked before saved inputs are merged or mutated.

Trusted Java callers may use `RunProcessAction.getState(...)` and `AsyncJobManager.getJobStatus(...)` internally.
Code serving external requests must use `getStateForUser(...)` and `getJobStatusForUser(...)` instead.
For custom asynchronous process starters, call `AsyncJobManager.withProcessUUID(...)` before `startJob(...)` to associate the job with its process.
Ownership is server-only and is not serialized into client responses. These scopes apply to live state in the current JVM; they do not add durable or shared process storage.

==== Cancellation and process lifecycle [[ProcessCancellation]]

`CancelProcessAction` runs the optional cancel step, subject to the same ownership and permission checks as process resume. Without a cancel step it logs a no-op. Existing missing/unknown-state handling remains; malformed UUID error classification is a separate Low issue (https://github.com/QRun-IO/qqq/issues/481[#481]).

The proposed one-shot/terminal lifecycle, active-callback signalling, completed/repeated cancellation rejection and concurrent/no-resume rules are deferred in https://github.com/QRun-IO/qqq/issues/478[#478]. The action does not establish those guarantees. Existing `AsyncJobManager.cancelJob` and `AsyncJobCallback.wasCancelRequested()` support cooperative work; custom code must observe the signal and release its owned resources. A cancel hook's exception remains visible to its caller.

New parent-to-child cancellation, publication boundaries and ETL boundary checks are deferred in https://github.com/QRun-IO/qqq/issues/479[#479] and https://github.com/QRun-IO/qqq/issues/480[#480]. No new callback helper API or forced interruption is introduced. See <<EtlCancellationTransactions,ETL cancellation and transactions>> for existing transaction limits.

==== Saved-view and saved-report ownership [[SavedAssetOwnership]]

Owned saved views and saved reports can be edited or deleted only by their stored owner. Submitting, clearing or omitting `userId` does not bypass this check, and an existing owned record cannot be transferred by editing that field. The check also applies to trusted inner actions using SYSTEM input; SYSTEM source alone is not an ownership bypass.

Mutations use a private native backend read, with the caller's transaction, to obtain ownership and report content. Hidden fields and returned-record presentation do not provide authorization values. The snapshot is not copied into action responses. Backends must supply the requested stored fields; missing or incomplete snapshots fail closed. No lock against concurrent external ownership changes is introduced.

Partial report updates validate the stored content merged with the submitted fields. They preserve omitted content and normalize an explicitly submitted query filter without writing old fields back. Deleting a report requires ownership but does not revalidate its JSON content. Existing null-owner behavior remains: such records retain their legacy maintenance semantics, and the report `USER_ID` default may assign the caller on update. Applications should assign an owner when creating private assets.

`storeSavedView` and `deleteSavedView` check the caller's insert/edit/delete table permissions and return process errors when the underlying action fails. A missing ID, denied mutation or invalid new view must not be treated as a successful save/delete.

The built-in saved-asset mutation policy remains owner-only for owned records. The generic `READ_WRITE` sharing label does not by itself grant another user permission to mutate these saved assets. Recipient visibility and sharing configuration are separate from this ownership check.

==== Sharing existing records [[SharedRecordAssetBinding]]

After applying the configured asset-owner check, `editSharedRecord` and `deleteSharedRecord` require `shareId` to identify a share whose configured asset foreign key matches `recordId`. A missing or mismatched pair returns an error before mutation, including when the same user owns both assets. Clients must retain the selected asset and its share ID together.

Both asset-owner and share-relationship checks require backend Count support; unsupported backends fail before mutation. Ownership is checked against the stored owner field and current user, rather than the returned display value. A hidden owner field is supported; removing it from personalized metadata can still deny the operation. The relationship check uses a filtered backend count, so hidden fields and returned-record presentation customizers do not supply the relationship used for this check. It does not make the check and later mutation atomic against concurrent reassignment. Applications still need appropriate record-security rules; saved views require explicit `ShareableTableMetaData` configuration for generic sharing. This check grants no new read or edit permissions.

==== Configured saved-view sharing [[ConfiguredSavedViewSharing]]

After `SavedViewsMetaDataProvider` defines the tables, configure `ShareableTableMetaData` on `savedView` with shared table `sharedSavedView`, asset field `savedViewId`, scope field `scope`, owner field `userId`, and a `user` audience using `userId`. The sample sharing fixture demonstrates this existing configuration API. Register `SharingMetaDataProvider` for the sharing processes. For private views, supply a user-level record lock on `userId`, using the caller's user ID as the security-key value, with the standard `READ_AND_WRITE` scope and `DENY` null policy. That configuration lets owners read and manage their views and shares; recipients can read granted views but cannot manage shares or edit/delete the owned view. Removing a share removes recipient access, even if a quick-view preference remains. Quick-view labels, counts and ordering are scoped to each user's preference record.

The default `user` audience accepts application-supplied user IDs. Configure an audience source table to validate recipient existence, and a possible-value source for an interactive user picker. Provider registration does not create a directory or authenticate users. The stock dashboard offers read-only sharing; the generic `READ_WRITE` enum remains available to applications but does not change built-in saved-asset ownership. Custom user-lock scopes and null policies retain their configured semantics: a WRITE-only lock does not restrict reads, and a READ-only lock does not provide write protection.

Joined record-security checks read native stored owner and join values privately with the caller's transaction. Hidden/heavy fields and returned-record display transformations cannot authorize a grant or prevent a valid owner grant. Declared null memory values retain the configured null policy; custom query backends must supply requested owner and join fields, and incomplete projections fail closed. No atomic guarantee against an external ownership change is added. Application and tenant security still require explicit configuration.

==== Saved-view dependent cleanup [[SavedViewDependentCleanup]]

Automatic dependent-share and quick-view preference cleanup is deferred in https://github.com/QRun-IO/qqq/issues/504[#504]. Saved-view deletion retains its established behavior; applications must manage any remaining dependent rows according to their own retention policy. No provider privacy-lock exemption or canonical audit-key fallback is introduced. Stored-owner authorization remains required.

==== Configured saved-report sharing [[ConfiguredSavedReportSharing]]

`SavedReportsMetaDataProvider` defines sharing metadata but does not install record-security locks. Configure them through its existing table enricher or before instance validation, and register `SharingMetaDataProvider` for the sharing processes. Bind the security key to the authenticated user's actual ID. This example adds the standard owner/recipient rules; keep any additional application or tenant locks:

[source,java]
----
String userKey = "savedReportUser";
String shareJoin = SavedReportsMetaDataProvider.SHARED_SAVED_REPORT_JOIN_SAVED_REPORT;
String ownerJoin = "savedReportJoinSharedSavedReport";
instance.addSecurityKeyType(new QSecurityKeyType().withName(userKey));
instance.addJoin(instance.getJoin(shareJoin).flip().withName(ownerJoin));
RecordSecurityLock user = new RecordSecurityLock()
   .withSecurityKeyType(userKey).withFieldName("userId");

instance.getTable(SavedReport.TABLE_NAME).withRecordSecurityLock(
   new MultiRecordSecurityLock().withOperator(MultiRecordSecurityLock.BooleanOperator.OR)
      .withLock(user.clone())
      .withLock(user.clone().withLockScope(RecordSecurityLock.LockScope.READ)
         .withFieldName("sharedSavedReport.userId").withJoinNameChain(List.of(shareJoin))));
instance.getTable(SharedSavedReport.TABLE_NAME).withRecordSecurityLock(
   new MultiRecordSecurityLock().withOperator(MultiRecordSecurityLock.BooleanOperator.OR)
      .withLock(user.clone().withFieldName("savedReport.userId").withJoinNameChain(List.of(ownerJoin)))
      .withLock(user.clone().withLockScope(RecordSecurityLock.LockScope.READ)));
----

The default lock scope is `READ_AND_WRITE` and its null policy is `DENY`. Owners can manage reports and all their grants; recipients can read shared reports and their own grant rows. Recipients cannot manage grants or edit/delete an owned report, including when a generic `READ_WRITE` label is stored. Multiple grants do not duplicate the report list. This configuration does not create an authenticated session, recipient directory or dashboard picker. Configure an audience source table when IDs must be validated against a directory.

Rendering uses the records selected for the process. Configure table record-security locks to limit that selection; the sharing recipe above applies those locks to owner and recipient reads. The proposed additional private-owner/grant-scope check and revalidation after a paused step are deferred in https://github.com/QRun-IO/qqq/issues/451[#451], with its candidate correctness issue https://github.com/QRun-IO/qqq/issues/446[#446]. Ownership rules for editing and deleting remain separate from permission to run a report.

Saved-report provider registration retains its existing associations; automatic dependent-share cleanup is deferred in https://github.com/QRun-IO/qqq/issues/504[#504]. This does not define schedule/job, rendered-history or archive retention. The native sample uses H2 for reports/grants, memory for history and owned filesystem storage for generated bytes.

==== First-party sharing fixture and dashboard limitations [[DashboardSharingDemo]]

The sample's explicit `qqq.sample.sharing=true` option requires `qqq.sample.mockAuthentication=true`. It supplies synthetic Alice, Bob and Casey sessions, native H2 asset/grant/directory rows, and the generic sharing processes. This local fixture is not production authentication and does not establish schedule/history/archive retention or complete output isolation.

Configure `audiencePossibleValueSourceName` on each shareable table and set the user audience's `sourceTableName` to the recipient directory. Directory primary keys are the authenticated IDs. The sample TABLE possible-value source uses `overrideIdField="audienceId"` to produce picker values such as `user:sample:bob`; the intended contract preserves the full ID after the first colon. The source table validates the ID and supplies its label. The dashboard currently grants `READ_ONLY`.

The dashboard path has deferred Medium/Low limitations: https://github.com/QRun-IO/qqq/issues/444[recipient ID truncation], https://github.com/QRun-IO/qqq/issues/445[missing-column preview failure], and https://github.com/QRun-IO/qqq/issues/371[mock identity]/https://github.com/QRun-IO/qqq/issues/332[display behavior]. Local fixes and dependent browser checks were preserved for a future release and removed from the active 4.0 candidate. Earlier passing browser results apply only to that preserved candidate. Native backend sharing/ownership fixtures remain separate evidence; the sample inventory retains outstanding acceptance gaps.

==== Scheduled reports [[ScheduledReportExecution]]

Scheduled reports establish an automated session for the stored user before rendering, including when invoked manually. Application-configured table/process permissions govern schedule management and selection. The added blanket owner-only CRUD/manual-run and nested-process permission policies are deferred in https://github.com/QRun-IO/qqq/issues/452[#452].
Nested rendering retains its trusted process default and forwards stored report inputs, including configured output values. It does not add a separate raw-table READ prerequisite merely because it runs on a schedule. Caller-session restoration remains deferred in https://github.com/QRun-IO/qqq/issues/450[#450].

The default automated session is a fresh ordinary `QSession` with a new UUID and the requested user ID. Only the selected backend-variant map is copied. Scheduler permissions, security keys, tokens, session identifiers, custom values and frontend values are not inherited. Mock, Auth0 and OAuth2 providers call `customizeAutomatedSessionForUser(...)` to establish owner-specific context. Integrations must load the owner's current grants and supply authentication that their provider can validate. Permissions alone do not create a valid Auth0/OAuth2 session; unsupported automation fails rather than borrowing the scheduler's credentials.

Requested email delivery uses configured sender and provider metadata. A later delivery failure can leave completed archive bytes while history records FAILED, retaining the established status behavior. Its proposed COMPLETE interpretation remains deferred in https://github.com/QRun-IO/qqq/issues/461[#461]. See <<ScheduledProcessFailures,scheduled process failures>> for the existing log-based scheduler boundary.
Record-backed activation, rescheduling, deactivation and deletion use explicitly configured linked jobs; see <<SavedReportScheduleLifecycle,saved-report schedule lifecycle>> for configuration and failure-reporting limits.
...
As such, in general, records can be processed a page at a time. Custom steps must also bound their caches and retained data to avoid exhausting server memory.
...
The following fragments share one enclosing application class. Supply `PROCESS_NAME`, replace each omitted QCodeReference target with the application's BackendStep class, and put the process-building statements inside the metadata producer method.

[source,java]
.Class fields and metadata-method statements for a customized linear flow:
----
...
// the additional steps get added via `withOptionalStep`, which only puts them in
// the process's stepMap, not its stepList!
process.withOptionalStep(new QFrontendStepMetaData().withName(STEP_A));
process.withOptionalStep(new QBackendStepMetaData().withName(STEP_B)
   .withCode(new QCodeReference(/*...*/)));
process.withOptionalStep(new QFrontendStepMetaData().withName(STEP_C));

process.withOptionalStep(new QBackendStepMetaData().withName(STEP_1)
   .withCode(new QCodeReference(/*...*/)));
process.withOptionalStep(new QFrontendStepMetaData().withName(STEP_2));
process.withOptionalStep(new QBackendStepMetaData().withName(STEP_3)
   .withCode(new QCodeReference(/*...*/)));
...
public static class StartStep implements BackendStep
{
  ...
  public void run(RunBackendStepInput runBackendStepInput, RunBackendStepOutput runBackendStepOutput) throws QException
  {
     ...
     if("letters".equals(runBackendStepInput.getValueString("which")))
     {
