...
The dashboard path has deferred Medium/Low limitations: https://github.com/QRun-IO/qqq/issues/444[recipient ID truncation], https://github.com/QRun-IO/qqq/issues/445[missing-column preview failure], and https://github.com/QRun-IO/qqq/issues/371[mock identity]/https://github.com/QRun-IO/qqq/issues/332[display behavior]. Local fixes and dependent browser checks were preserved for a future release and removed from the 4.0 release. Earlier passing browser results apply only to that preserved candidate. Native backend sharing/ownership fixtures remain separate evidence; the sample inventory retains outstanding acceptance gaps.
