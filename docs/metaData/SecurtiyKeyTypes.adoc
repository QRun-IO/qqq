...
(as values in fields).
In this default `READ_AND_WRITE` example, access requires satisfying the record's applicable security locks.
A lock can instead apply only to reads or writes; see the `lockScope` properties under {link-table}. Table permissions remain independent.
...
==== All Access Key
A Security Key Type's `allAccessKeyName` satisfies locks of that type when the session contains the named key with a value of `true`.
This satisfies that lock even as an alternative within an OR group; other required lock branches and table permissions still apply.
