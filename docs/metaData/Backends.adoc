...
* MongoDB document storage, via `MongoDBBackendMetaData`
* PostgreSQL and SQLite through their dedicated RDBMS backend modules
* <<MemoryBackend,In-Memory data stores>>
...
* `enabledCapabilities` and `disabledCapabilities` - *Sets*, containing *Capability* enum values.
...
** When the backend disables a capability, a table-level enabled entry overrides that exclusion. Otherwise, a table-level disabled entry disables it. Avoid contradictory enabled and disabled entries. A flag cannot add an operation that the selected backend implementation does not implement.
** Capability flags control the operations exposed in frontend metadata. The count and export HTTP routes refuse tables without `TABLE_COUNT` or `TABLE_EXPORT`; other direct table HTTP operations do not enforce these flags, so use `permissionRules` to deny access. Broader runtime capability enforcement is deferred in https://github.com/QRun-IO/qqq/issues/546[issue #546].
...
** Configure `backendVariantsConfig`: its variant type key, options table/filter or lookup function, and mapping from backend settings to fields on a variant record. See {qqq-source-url}/qqq-backend-core/src/main/java/com/kingsrook/qqq/backend/core/model/metadata/variants/BackendVariantsConfig.java[BackendVariantsConfig] and validate the configuration with QInstanceValidator.
...
See {qqq-source-url}/qqq-backend-module-api/src/main/java/com/kingsrook/qqq/backend/module/api/actions/BaseAPIActionUtil.java[BaseAPIActionUtil] for the complete override contract and {qqq-tree-url}/qqq-backend-module-api/src/test/java[module tests] for request/response examples.
...
* `customValues` - *Map of String → Serializable* - Application-defined additional name=value pairs available to the backend action utility.
...
** `API_TOKEN` - Uses the `apiKey` property in an `Authorization` header, as: `"Token " + apiKey`
** `BASIC_AUTH_API_KEY` - Uses the `apiKey` property, Base64-encoded, in an `Authorization` header, as `"Basic " + base64(apiKey)`
** `BASIC_AUTH_USERNAME_PASSWORD` - Combines the `username` and `password` properties, Base64-encoded, in an `Authorization` header, as `"Basic " + base64(username + ":" + password)`
...
The `memory` backend stores QRecords in the process. Its Query, Count and Aggregate paths use active personalized READ rules, including supported INNER and LEFT joins. Joined READ checks run before LEFT null extension. These operations do not imply SQL support for every operator, join type or value conversion.

Query honors selected join aliases, `fieldNamesToInclude` and `shouldFetchHeavyFields`; see <<QueryInput>>. Supported joined virtual fields use alias-local function inputs, and Boolean subfilters retain the same execution context. Virtual selection, filtering, sorting and Aggregate operands retain their function-specific behavior. Unselected joined fields are not returned. Count can use a private key for distinct counting without restoring it to public metadata. Aggregate can read heavy operands internally; its computed results do not receive Query result masking.

Memory Aggregate retains Java equality for groups/distinct values and double-based DECIMAL AVG. Decimal scale and array identity can distinguish values, and precision differs from native SQL. See <<AggregateAction>> for source-window, empty-result, type and provider boundaries.

The legacy `MemoryRecordStore` setting `buildJoinCrossProductFromJoinContext=false` is no longer an executable query mode: reads fail with a checked `QException`. The getter, setter, fluent setter and `BUILD_JOIN_CROSS_PRODUCT_FROM_JOIN_CONTEXT_DEFAULT` field remain available. Keep the default true and remove any configuration that selects false; true uses the join plan that includes security rules.
