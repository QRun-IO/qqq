...
Examples below are application fragments: imports and explicitly omitted application classes must be supplied. `AuthScope` is `com.kingsrook.qqq.backend.core.model.metadata.authentication.AuthScope`. Fully anonymous authentication illustrates metadata registration; configure an appropriate authentication provider and permissions before deploying an application.
...
* `QBackendMetaData` - a backend data store.
...
{
    QInstance qInstance = new QInstance();
    qInstance.registerAuthenticationProvider(AuthScope.instanceDefault(), defineAuthenticationMetaData());
...
{
    QInstance qInstance = new QInstance();
    qInstance.registerAuthenticationProvider(AuthScope.instanceDefault(), new AuthMetaDataProvider().defineAuthenticationMetaData());
...
(technically, any class which implements `TopLevelMetaDataInterface`).  Implementers of the interface are then
...
=== Failing fast on producer errors

By default, `MetaDataProducerHelper` logs a warning and skips any class it finds but can't use as a
producer, and any producer whose `produce` method throws.  The application then starts without that
meta-data.  Classes it can't use include:

* a `MetaDataProducerInterface` without a public no-arg constructor, or whose constructor throws;
* a `@QMetaDataProducingEntity` class that is not a `QRecordEntity`, or whose `TABLE_NAME` is not a
`String`;
* a `@QMetaDataProducingPossibleValueEnum` enum that is not a `PossibleValueEnum`;
* a `@ChildTable` that asks for a `@ChildRecordListWidget` without its `@ChildJoin`, or whose child
entity has no field with a possible value source naming the parent table.

To make those errors stop startup instead, turn on fail-fast before processing producers, either on
the `QInstance`, or with the system property `qqq.metaData.failOnProducerError=true`.  With either
one on, the helper throws a `QException` naming the failing class, with the original error as its cause.

[source,java]
.Failing fast on meta-data producer errors
----
QInstance qInstance = new QInstance()
...
An `AbstractMetaDataProducerBasedQQQApplication` (such as `MetaDataProducerBasedQQQApplication`) creates
its own `QInstance`, so set the flag on the application with `withFailOnMetaDataProducerError(true)`, or
on the `QApplicationLauncherConfig` that launches it (see <<Javalin>>).
...
public class MyMultiProducer implements MetaDataProducerInterface<MetaDataProducerMultiOutput>
{
    ...
    public MetaDataProducerMultiOutput produce(QInstance qInstance)
    {
       ...
       output.add(new QBackendMetaData()
          .withName("memoryBackend")
          .withBackendType(MemoryBackendModule.class));
...
At this point, let's take a brief aside to dig deeper into the creation of a `QTableMetaData` object.
...
in `@QMetaDataProducingEntity.tableMetaDataCustomizer`, when an application calls
      ...
      public QTableMetaData customizeMetaData(QInstance qInstance, QTableMetaData table) throws QException
      {
         ...
         table
            ...
            .withSection(new QFieldSection("identity", new QIcon().withName("badge"), Tier.T1,
               List.of("id", "name")))
            // add sections for the remaining application fields
