...
** Use the backend module's table-details class; see the module README and metadata source for its storage-specific fields.
...
[source,java]
----
// MyPreInsCustomizer is the application class extending AbstractPreInsertCustomizer.
table.withCustomizer(TableCustomizers.PRE_INSERT_RECORD, new QCodeReference(MyPreInsCustomizer.class));
----
...
* `searchFields` - *List of String* - Fields that record search matches a search term against (`RecordSearchAction`, and the v1 `POST /search` endpoint that the dashboard's global search uses).
String fields (`STRING`, `TEXT`, `HTML`) match when they contain the term, ignoring case; integer fields (`INTEGER`, `LONG`) match when they equal a numeric term.
A table without search fields is not searched.
Search fields must be visible, non-password fields of the table, and the table's backend must not use variants.
Record search only searches tables the user may read, applies record security locks, and returns a limited number of records per table.
The table meta data lists `searchFields` only to users who may read the table, so a frontend searches only when some table declares them.

[source,java]
----
.withSearchFields("firstName", "lastName", "email")
----
...
The `supplementalMetaData` map attaches module-specific table settings. For application APIs, see {qqq-source-url}/qqq-middleware-api/src/main/java/com/kingsrook/qqq/api/model/metadata/tables/ApiTableMetaData.java[ApiTableMetaData].


==== Unique-key validation

QQQ checks declared unique keys before INSERT and key-changing UPDATE. Sparse updates retain omitted key components for validation; an explicit null remains null, and any null component is distinct. Unchanged self keys are allowed, but another stored owner is a conflict, including owners hidden by record security. Validation does not grant READ access or return stored key values. Within a request, accepted typed key tuples remain reserved for later records. If repeated updates target the same primary key and any fragment changes a key, every occurrence is rejected; combine those fragments into one record. Non-key-only updates retain their existing behavior.

Native backend lookup must provide complete stored-key visibility. Custom backend query implementations provide `lookupUniqueKey(UniqueKeyLookup.Input)`; API providers implement the corresponding method on their configured `BaseAPIActionUtil`. Unsupported lookups fail before writing. These checks compare typed values before backend conversion; matching storage constraints are still needed for final stored-value uniqueness under concurrent writes, numeric rounding and database collations. INSERT customizers after the unique-key phase remain trusted and must preserve the constraint. The existing trusted INSERT `skipUniqueKeyCheck` option retains its behavior.
...
** `codeReference` - *QCodeReference, Conditional* - reference to a class that implements `RecordAutomationHandlerInterface`, to be executed as the custom-code of the action.
...
INSERT and UPDATE can process associated records supplied with the parent; replacement UPDATE also deletes omitted stored members. Cascading DELETE follows the canonical declared association graph and complete stored membership, independently of supplied child records or UI visibility. See <<DeleteAction>> for selection, results and transaction boundaries.
...
Full frontend table metadata exposes each association as `name`, `associatedTableName` and its resolved `join`, including the declared `joinOns` field pairs. Names are independent of target table names: `pets` may target `pet`, and multiple groups may target the same table. Use the exact association name to index `associatedRecords`; query `exposedJoins` are a separate contract. Child metadata and permission flags remain independent. Managed widget sections bind through `widgetName` and the widget's frontend defaults (`manageAssociationName` for ChildRecordList or `associationName` for RowBuilder).

A successfully queried association is returned as an array, including `[]` when that group has no records. Base-only, unrequested and personalized-away groups remain absent; a failed query does not manufacture an empty group. Preserve this distinction in write clients: omitting a group leaves it unchanged, while an explicit empty replacement removes that group's stored children subject to the existing write permissions.

Expanded reads use the declared parent and child endpoints in either join direction. Each relationship tuple is converted to the child's declared field types before matching; a tuple containing null has no read membership. Required relationship fields missing from query results cause an error instead of a misleading empty group. Nested association selections use literal names, including names containing regular-expression characters. Nested INSERT and UPDATE use the same oriented field pairs. Replacement UPDATE finds omitted children using the stored old relationship, then assigns retained/new children the normalized prospective values. Omitted parent components remain stored, including hidden/heavy fields; private fetched values are not added to the public patch or child response. A null old tuple has no members. A null prospective tuple permits an empty replacement, but rejects child writes before parent mutation. Cascading DELETE uses the same declared tuples and complete native membership, deleting descendants before ancestors; UPDATE removes omitted children before changing their parent. An incomplete or rejected child deletion stops that parent mutation. Cyclic cascades are rejected. Caller transactions provide rollback where supported; these operations do not create an automatic transaction. Custom backend requirements and remaining recursive HTTP copy limits are described in the migration guide.
...
** `WRITE` - restrict writes to users satisfying this lock; this lock does not restrict reads.
** `READ` - restrict reads to users satisfying this lock; this lock does not itself restrict writes.
Record-lock scope does not grant table permissions or bypass other applicable locks. Nested `MultiRecordSecurityLock` AND/OR groups retain their Boolean structure after filtering for the operation's applicable scopes.
...
Currently, only `UNIQUE_KEY_TO_UNIQUE_KEY` loads records from the source. The two primary-key-based modes below are declared but unfinished, and `cacheSourceMisses` has no effect. These existing limitations are deferred under https://github.com/QRun-IO/qqq/issues/545[issue #545]. Configure record-security rules on both source and cache tables when access is restricted; caching does not supply those application policies.
